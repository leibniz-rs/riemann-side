...
\subsection{Statement of the Main Result (Unconditional)}

The combination of the product-certificate construction, the VK density inputs, the boundary wedge geometry, and a rigorous constants audit yields the following main theorem.
...
Proposition~\ref{prop:closure} shows that if $\Upsilon:=\dfrac{C_{\mathrm{geom}}\sqrt{K_\xi}}{\pi\,c_{\mathrm{plateau}}}<\tfrac12$, then $\re(2J)\ge 0$ almost everywhere on the boundary and the Schur pinch yields RH. In Section~\ref{subsec:constants-audit} we perform a rigorous constants audit (VK constants, tail BMO bound, geometric constants) and certify $\Upsilon<\tfrac12$. The conclusion follows.
\end{proof}
...
\noindent
Combining Proposition~\ref{prop:closure} with the certified bound completes the wedge step and closes the proof of Theorem~\ref{thm:main}.
...
Specifically, consider the function $f(s) = J(s) + 1$. Its real part is non-negative on the boundary. Unless $f$ is identically zero (which it is not, as $J(s) \to 1$ at infinity by Proposition 2.3(iv)), the Minimum Principle for harmonic functions implies that $\re f(s) > 0$ everywhere in the interior of its domain of holomorphy. Thus,
\begin{equation}
