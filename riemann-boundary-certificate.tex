...
\begin{equation} \label{eq:J_def}
    J(s) := \frac{\det\nolimits_2(I - A(s))}{\mathcal{O}_\zeta(s) \, \zeta(s)}.
\end{equation}
...
\begin{enumerate}
    \item \textbf{Holomorphy:} $J(s)$ is meromorphic in $\Omega_{1/2}$. Its poles and zeros coincide exactly with the zeros of $\zeta(s)$ (with multiplicities determined by the order of the zero and the regularization terms).
...
From Proposition \ref{prop:closure}, we know that the boundary values of the product-certificate satisfy $\re J(1/2+it) \ge 0$ for almost all $t$. Since $J(s)$ is meromorphic in the half-plane $\Omega_{1/2}$, we can appeal to the Herglotz representation theorem (or standard Poisson transport for functions with positive real part).
...
\begin{equation}
    \re J(s) > 0 \quad \text{for all } s \in \Omega_{1/2} \setminus \mathcal{Z},
\end{equation}
where $\mathcal{Z}$ is the set of poles of $J$ (the zeros of $\zeta$). This positivity allows us to define the Schur transform $\Theta(s)$ globally.
