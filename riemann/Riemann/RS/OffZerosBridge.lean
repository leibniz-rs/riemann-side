...
/-! ## Complex inverse limit lemmas

These lemmas help prove that 1/f(z) → 0 when ‖f(z)‖ → ∞ for complex functions. -/

/-- If ‖f(z)‖ → ∞ along a filter, then (f(z))⁻¹ → 0. -/
theorem tendsto_inv_of_norm_tendsto_atTop
    {α : Type*} {l : Filter α} {f : α → ℂ}
    (hf : Tendsto (fun x => ‖f x‖) l atTop) :
    Tendsto (fun x => (f x)⁻¹) l (𝓝 0) := by
  -- Key fact: ‖z⁻¹‖ = ‖z‖⁻¹ for z ≠ 0
  rw [Metric.tendsto_nhds]
  intro ε hε
  -- Since ‖f x‖ → ∞, eventually ‖f x‖ > 1/ε, so ‖f x‖⁻¹ < ε
  have h1 : ∀ᶠ x in l, (1 / ε) < ‖f x‖ := by
    have hε_inv_pos : 0 < 1 / ε := one_div_pos.mpr hε
    exact hf.eventually_gt_atTop (1 / ε)
  ...
  have hf_ne : f x ≠ 0 := by
    intro hf0
    simp only [hf0, norm_zero] at hx
    have : 0 < 1 / ε := one_div_pos.mpr hε
    linarith
  have hfx_pos : 0 < ‖f x‖ := norm_pos_iff.mpr hf_ne
  -- Since 1/ε < ‖f x‖, we have ‖f x‖⁻¹ < ε
  -- Using: 1/a < 1/b ↔ b < a for positive a, b
  rw [inv_eq_one_div]
  -- Use one_div_lt_one_div: 1/a < 1/b ↔ b < a (for positive a, b)
  -- We need: 1/‖f x‖ < ε, i.e., 1/‖f x‖ < 1/(1/ε), i.e., 1/ε < ‖f x‖
  rw [show ε = 1 / (1 / ε) by field_simp]
  exact (one_div_lt_one_div hfx_pos (one_div_pos.mpr hε)).mpr hx

/-- nhdsWithin variant: if ‖f(z)‖ → ∞ along nhdsWithin, then (f(z))⁻¹ → 0. -/
theorem tendsto_inv_of_norm_tendsto_atTop_nhdsWithin
    {U : Set ℂ} {ρ : ℂ} {f : ℂ → ℂ}
    (hf : Tendsto (fun z => ‖f z‖) (nhdsWithin ρ U) atTop) :
    Tendsto (fun z => (f z)⁻¹) (nhdsWithin ρ U) (𝓝 0) :=
  tendsto_inv_of_norm_tendsto_atTop hf

/-- Constant multiple variant: if ‖f(z)‖ → ∞, then c/(f(z)) → 0 for any constant c. -/
theorem tendsto_const_div_of_norm_tendsto_atTop
    {α : Type*} {l : Filter α} {f : α → ℂ} {c : ℂ}
    (hf : Tendsto (fun x => ‖f x‖) l atTop) :
    Tendsto (fun x => c / f x) l (𝓝 0) := by
  have h_inv := tendsto_inv_of_norm_tendsto_atTop hf
  have h_mul : Tendsto (fun x => c * (f x)⁻¹) l (𝓝 (c * 0)) :=
    h_inv.const_mul c
  simp only [mul_zero] at h_mul
  have h_eq : (fun x => c / f x) = (fun x => c * (f x)⁻¹) := by
    funext x
    rw [div_eq_mul_inv]
  rw [h_eq]
  exact h_mul

/-- If f is continuous, f(ρ) ≠ 0, and g tends to 0 at ρ with g(z) ≠ 0 on a punctured
neighborhood, then ‖f(z)/g(z)‖ → ∞ as z → ρ. -/
theorem tendsto_norm_div_atTop_of_tendsto_zero
    {U : Set ℂ} {ρ : ℂ} {f g : ℂ → ℂ}
    (hf_cont : ContinuousAt f ρ)
    (hf_ne : f ρ ≠ 0)
    (hg_tendsto : Tendsto g (nhdsWithin ρ U) (𝓝 0))
    (hg_ne : ∀ᶠ z in nhdsWithin ρ U, g z ≠ 0) :
    Tendsto (fun z => ‖f z / g z‖) (nhdsWithin ρ U) atTop := by
  -- We show that ‖f(z)/g(z)‖ → ∞ using:
  -- ‖f(z)/g(z)‖ = ‖f(z)‖/‖g(z)‖
  -- where ‖f(z)‖ → ‖f(ρ)‖ ≠ 0 and ‖g(z)‖ → 0
  rw [Filter.tendsto_atTop]
  intro M
  -- Since ‖f(ρ)‖ > 0, find δ > 0 such that ‖f(z)‖ > ‖f(ρ)‖/2 in a neighborhood
  have hfρ_pos : 0 < ‖f ρ‖ := norm_pos_iff.mpr hf_ne
  have hfρ_half_pos : 0 < ‖f ρ‖ / 2 := by positivity
  -- f is continuous at ρ, so ‖f‖ is continuous at ρ
  have hf_norm_cont : ContinuousAt (fun z => ‖f z‖) ρ := continuous_norm.continuousAt.comp hf_cont
  -- Eventually ‖f(z)‖ > ‖f(ρ)‖/2
  have h_f_lower : ∀ᶠ z in nhdsWithin ρ U, ‖f ρ‖ / 2 < ‖f z‖ := by
    have h1 : ∀ᶠ z in 𝓝 ρ, ‖f z‖ ∈ Set.Ioo (‖f ρ‖ / 2) (‖f ρ‖ * 2) := by
      apply hf_norm_cont.eventually
      exact Ioo_mem_nhds (by linarith) (by linarith)
    filter_upwards [nhdsWithin_le_nhds h1] with z hz
    exact hz.1
  -- Since g → 0, eventually ‖g(z)‖ < ‖f(ρ)‖ / (2 * (max M 0 + 1))
  let M' := max M 0
  have hM'_pos : 0 ≤ M' := le_max_right M 0
  have hbound : 0 < ‖f ρ‖ / (2 * (M' + 1)) := by positivity
  have h_g_upper : ∀ᶠ z in nhdsWithin ρ U, ‖g z‖ < ‖f ρ‖ / (2 * (M' + 1)) := by
    have h1 : Tendsto (fun z => ‖g z‖) (nhdsWithin ρ U) (𝓝 0) := by
      have h := hg_tendsto.norm
      simp only [norm_zero] at h
      exact h
    exact (Metric.tendsto_nhds.mp h1 (‖f ρ‖ / (2 * (M' + 1))) hbound).mono fun z hz => by
      rw [Real.dist_eq, sub_zero, abs_of_nonneg (norm_nonneg _)] at hz
      exact hz
  -- Combine: when both hold and g ≠ 0, we have ‖f/g‖ > M
  filter_upwards [h_f_lower, h_g_upper, hg_ne] with z hf_lb hg_ub hg_nez
  have hgz_pos : 0 < ‖g z‖ := norm_pos_iff.mpr hg_nez
  rw [norm_div]
  -- Show ‖f z‖ / ‖g z‖ ≥ M
  have hfz_pos : 0 < ‖f z‖ := lt_of_lt_of_le hfρ_half_pos hf_lb.le
  -- Key inequalities:
  -- hf_lb : ‖f ρ‖ / 2 < ‖f z‖
  -- hg_ub : ‖g z‖ < ‖f ρ‖ / (2 * (M' + 1))
  -- We want: M ≤ ‖f z‖ / ‖g z‖
  -- Note: M ≤ M' and (‖f ρ‖/2) / (‖f ρ‖/(2*(M'+1))) = M' + 1
  have h_ratio : (‖f ρ‖ / 2) / (‖f ρ‖ / (2 * (M' + 1))) = M' + 1 := by
    have hne : ‖f ρ‖ ≠ 0 := norm_ne_zero_iff.mpr hf_ne
    field_simp
  -- Since ‖f z‖ > ‖f ρ‖/2 and ‖g z‖ < ‖f ρ‖/(2*(M'+1)):
  -- ‖f z‖ / ‖g z‖ > (‖f ρ‖/2) / (‖f ρ‖/(2*(M'+1))) = M' + 1 > M
  have h1 : ‖f z‖ / ‖g z‖ > (‖f ρ‖ / 2) / (‖f ρ‖ / (2 * (M' + 1))) := by
    have ha : ‖f ρ‖ / 2 < ‖f z‖ := hf_lb
    have hb : ‖g z‖ < ‖f ρ‖ / (2 * (M' + 1)) := hg_ub
    have hc : 0 < ‖f ρ‖ / (2 * (M' + 1)) := hbound
    -- (‖f ρ‖/2) / (‖f ρ‖/(2*(M'+1))) < ‖f z‖ / ‖g z‖
    -- This follows from: a/b < c/d when a < c and d < b (for positive values)
    -- i.e., a*d < c*b
    rw [gt_iff_lt, div_lt_div_iff₀ hc hgz_pos]
    calc (‖f ρ‖ / 2) * ‖g z‖
      < (‖f ρ‖ / 2) * (‖f ρ‖ / (2 * (M' + 1))) := mul_lt_mul_of_pos_left hb hfρ_half_pos
      _ < ‖f z‖ * (‖f ρ‖ / (2 * (M' + 1))) := mul_lt_mul_of_pos_right ha hc
  have h2 : M' + 1 < ‖f z‖ / ‖g z‖ := by rw [h_ratio] at h1; exact h1
  have h3 : M ≤ M' := le_max_left M 0
  linarith
